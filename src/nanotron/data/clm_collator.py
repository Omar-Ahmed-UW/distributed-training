...
class DataCollatorForCLMWithPositionIds:
    ...
    def __call__(self, examples: List[Dict[str, List[np.ndarray]]]) -> Dict[str, Union[torch.Tensor, TensorPointer]]:
        ...
        current_pp_rank = dist.get_rank(self.parallel_context.pp_pg)
        cp_size = self.parallel_context.context_parallel_size # Get cp_size early

        if current_pp_rank not in [self.input_pp_rank, self.output_pp_rank]:
            assert all(len(example) == 0 for example in examples)
            return {
                "input_ids": TensorPointer(group_rank=self.input_pp_rank),
                "position_ids": TensorPointer(group_rank=self.input_pp_rank), # Correct key name
                "label_ids": TensorPointer(group_rank=self.output_pp_rank),
                "label_mask": TensorPointer(group_rank=self.output_pp_rank),
            }
        ...
        if current_pp_rank == self.input_pp_rank:
            ...
            if "positions" in examples[0] and self.use_doc_masking:
                ...
                result["position_ids"] = position_ids[:, :-1] # Assign to 'position_ids'
            else:
                # Default: sequential position ids
                result["position_ids"] = np.arange(self.sequence_length)[None, :].repeat(batch_size, axis=0) # Assign to 'position_ids'
            ...
            # result["position_ids"] = result.pop("positions") # No need to pop if assigned correctly above
        ...
        if current_pp_rank == self.output_pp_rank:
            ...
            # Context Parallelism: Each CP rank gets a slice of the label_ids and label_mask
            if cp_size > 1:
                cp_rank = dist.get_rank(self.parallel_context.cp_pg) # Define cp_rank here as well
                local_slice = slice(
                    cp_rank * self.sequence_length // cp_size, (cp_rank + 1) * self.sequence_length // cp_size
                )
                result["label_ids"] = result["label_ids"][:, local_slice]  # (b, s/cp_size)
        ...
        # Validate shapes based on whether CP is active
        expected_sequence_length = self.sequence_length // cp_size if cp_size > 1 else self.sequence_length

        if (
            not isinstance(result["input_ids"], TensorPointer)
            and result["input_ids"].shape[-1] != expected_sequence_length
        ):
            raise ValueError(
        ...
        if (
            not isinstance(result["label_ids"], TensorPointer)
            and result["label_ids"].shape[-1] != expected_sequence_length
        ):
            raise ValueError(
