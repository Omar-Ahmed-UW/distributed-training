...
class Qwen2Model(nn.Module):
    ...
    def forward(
        ...
        resolved_position_ids = output.get("position_ids")
        if resolved_position_ids is not None and not isinstance(resolved_position_ids, TensorPointer):
            if resolved_position_ids.numel() > 0:
                start_indices = torch.where(resolved_position_ids.view(-1) == 0)[0]
                # Get the total number of elements in the tensor
                numel_tensor = torch.tensor([resolved_position_ids.numel()], dtype=torch.int32, device=start_indices.device)
                # Concatenate start indices and the total count to form cu_seqlens
                cu_seqlens = torch.cat([start_indices, numel_tensor]).to(torch.int32)

        decoder_states = {
            "hidden_states": output["input_embeds"],
            "position_ids": resolved_position_ids, # This might be None or TensorPointer (previous checks handled TensorPointer)
            "cu_seqlens": cu_seqlens,         # This might be None
        }
