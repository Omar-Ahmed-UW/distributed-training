        ...
        if dist.get_rank(self.p2p.pg) != self.rank:
            ...
            for name, tensor in sorted_kwargs:
                if isinstance(tensor, TensorPointer):
                    ...
                    assert isinstance(tensor, torch.Tensor) or tensor is None, f"Expected Tensor or None for key '{name}' on non-compute rank, got {type(tensor)}"                    # We need to send the tensor to the rank that actually runs the compute
                    
                    # We might need to send the tensor or None to the rank that actually runs the compute
                    if tensor is None:
                        # Need a way to signal None to the receiving rank if necessary,
                        # or ensure the receiver handles missing keys if None isn't sent.
                        # This part depends on how the receiving logic expects None.
                        # Assuming for now that None doesn't need explicit sending,
                        # but this might need adjustment based on P2P implementation.
                        continue # Skip sending if it's None? Or send a placeholder?
