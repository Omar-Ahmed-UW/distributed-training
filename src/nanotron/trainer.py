...
class DistributedTrainer:
    ...
    def training_step(
        ...
        if self.parallel_context.pp_pg.size() > 1:
            # Create tensor placeholders on all ranks
            
            if loss_avg is not None and z_loss_avg is not None:
                pass

            if loss_avg is None:
                loss_avg = torch.tensor(0.0, device="cuda")
            if z_loss_avg is None and "z_loss" in outputs[0]:
                z_loss_avg = torch.tensor(0.0, device="cuda")
            
            # Use broadcast to share loss from output rank to all PP ranks
            src_rank = self.parallel_context.get_global_rank(
                pp_rank=self.unwrapped_model.output_pp_rank,
                dp_rank=0, tp_rank=0, ep_rank=0, cp_rank=0
            ).item()
            dist.broadcast(loss_avg, src=src_rank, group=self.parallel_context.pp_pg)

            if z_loss_avg is not None:
                dist.broadcast(z_loss_avg, src=src_rank, group=self.parallel_context.pp_pg)
